...
// Customer migration and completion can run in separate requests.
// Reconnect before any post-update database escaping or queries.
if ($_SESSION['step'] >= 4) {
    hesk_dbConnect();
}
...
function hesk_iFinish()
{
    ...
    // Normalize invalid customer email values created by older Hesk versions.
    // This runs after any legacy customer migration has completed.
    hesk_dbQuery("UPDATE `".hesk_dbEscape($hesk_settings['db_pfix'])."customers` SET `email` = '' WHERE `email` = '0'");
...
function hesk_iUpdateTables()
{
    ...
    // 3.7.12 Separate brute force counters by user realm
    if ($update_all_next || $hesk_settings['update_from'] == '3.7.8') {
        hesk_dbQuery("TRUNCATE TABLE `".hesk_dbEscape($hesk_settings['db_pfix'])."logins`");
        hesk_dbQuery("ALTER TABLE `".hesk_dbEscape($hesk_settings['db_pfix'])."logins` DROP INDEX `ip`, ADD `scope` VARCHAR(32) COLLATE utf8_unicode_ci NOT NULL DEFAULT 'GENERAL' AFTER `ip`, ADD UNIQUE KEY `ip_scope` (`ip`, `scope`)");
        $update_all_next = 1;
    }
...
function hesk_iSaveSettings()
{
    ...
    foreach ($hesk_default as $k => $v)
    {
        ...
        elseif (is_int($v))
        {
            // Numeric settings must remain valid PHP values even if an old settings file stored an empty string
            $hesk_settings[$k] = intval($hesk_settings[$k]);
        }
    }
...
function hesk_iDetectVersion()
{
    ...
    {
        // Version 3.7.12 tables installed?
        $res = hesk_dbQuery("SHOW COLUMNS FROM `".hesk_dbEscape($hesk_settings['db_pfix'])."logins` LIKE 'scope'");
        if (hesk_dbNumRows($res))
        {
            return '3.7.12';
        }
