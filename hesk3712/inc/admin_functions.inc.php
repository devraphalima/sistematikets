...
function hesk_isLoggedIn()
{
    ...
    else
    {
		...
		$res = hesk_dbQuery( "SELECT `id`,`user`, `pass`, `isadmin`, `categories`, `heskprivileges`, `signature`, `mfa_enrollment` FROM `".$hesk_settings['db_pfix']."users` WHERE `id` = '".intval($_SESSION['id'])."' AND `active` = 1 LIMIT 1" );
		...
		$_SESSION['mfa_enrollment'] = intval($me['mfa_enrollment']);
		if (empty($_SESSION['mfa_enrollment']) && $hesk_settings['require_mfa'])
		{
