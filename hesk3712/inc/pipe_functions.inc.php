...
function hesk_email2ticket($results, $protocol = 0, $set_category = 1, $set_priority = -1)
{
    ...
    $use_reply_to = isset($results['reply-to'][0]['address']);

    if ($hesk_settings['customer_accounts'] && $hesk_settings['customer_accounts_required'])
    {
        // Required customer accounts must use "From:" for identity
    ...
    if ($use_reply_to)
    {
        $tmpvar['email'] = hesk_validateEmail($results['reply-to'][0]['address'],'ERR',0);
    }
    elseif (isset($results['from'][0]['address']))
    {
    ...
    if ( ! $customer_account_required &&
        isset($results['reply-to'][0]['name']) && strlen($results['reply-to'][0]['name']) )
    {
    ...
    $tmpvar['email_id'] = isset($results['Message-ID']) && is_string($results['Message-ID']) ? trim($results['Message-ID']) : '';
    $tmpvar['email_reply_to'] = isset($results['In-Reply-To']) && is_string($results['In-Reply-To']) ? trim($results['In-Reply-To']) : '';
