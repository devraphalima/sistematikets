...
$res = hesk_dbQuery("SELECT `number` FROM `".hesk_dbEscape($hesk_settings['db_pfix'])."logins` WHERE `ip`='".hesk_dbEscape(hesk_getClientIP())."' AND `scope`='TICKET' AND `last_attempt` IS NOT NULL AND DATE_ADD(`last_attempt`, INTERVAL ".intval($hesk_settings['attempt_banmin'])." MINUTE ) > NOW() LIMIT 1");
...
if (hesk_verifyEmailMatch($trackingID, $my_email, $customers) !== true)
{
...
if (hesk_dbNumRows($res) > 0)
{
	...
	{
		hesk_dbQuery("INSERT INTO `".hesk_dbEscape($hesk_settings['db_pfix'])."logins` (`ip`, `scope`, `number`) VALUES ('".hesk_dbEscape(hesk_getClientIP())."', 'TICKET', ".intval($hesk_settings['attempt_limit'] + 1).") ON DUPLICATE KEY UPDATE `number`=VALUES(`number`), `last_attempt`=NOW()");
