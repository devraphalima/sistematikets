...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (hesk_POST('a') === 'backup_email') {
        ...
        $skip_email = true;
        $posted_mfa_method = hesk_POST('mfa-method');
        $mfa_method = $posted_mfa_method === 'PASSWORD' ? 0 : ($posted_mfa_method === 'EMAIL' ? 1 : ($posted_mfa_method === 'AUTH-APP' ? 2 : -1));

        // Only allow the account's configured method, plus email as the
        // documented fallback for authenticator-app users.
        if ($mfa_method !== $mfa_enrollment && !($mfa_enrollment === 2 && $mfa_method === 1)) {
            hesk_forceLogoutCustomer($hesklang['invalid_action']);
        }
        ...
        if ($mfa_method === 0) {
            ...
            if ( ! $pass) {
                ...
                hesk_limitInternalBfAttempts('CUSTOMER');
            ...
            hesk_limitInternalBfAttempts('CUSTOMER');
            if (($mfa_method === 1 && is_mfa_email_code_valid($_SESSION['customer']['id'], hesk_POST('verification-code'), 'CUSTOMER')) ||
                ($mfa_method === 2 && is_mfa_app_code_valid($_SESSION['customer']['id'], hesk_POST('verification-code'), null, 'CUSTOMER'))) {
                hesk_cleanBfAttempts('CUSTOMER');
                handle_successful_elevation();
            } else {
                // Verification failed
