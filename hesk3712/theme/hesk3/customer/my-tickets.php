...
// Remove invalid and unavailable columns before rendering the table
...
// Tracking ID and subject already link to ticket details. Link the sequential ID only
// when neither of those columns is available; otherwise keep it as plain text.
...
// If none of the linkable columns can be displayed, add a dedicated fallback link column.
            ...
            <div class="contr">
                ...
                <div class="table-wrap">
                    <table id="default-table" class="table sindu-table">
                        <thead>
                        <tr>
                            <?php foreach ($hesk_settings['customer_ticket_list'] as $field): ?>
                            ...
                            <?php if ($show_view_ticket_column): ?>
                            <th><?php echo $hesklang['view_ticket']; ?></th>
                            <?php endif; ?>
                        </tr>
                        </thead>
                        <tbody>
                        <?php if (count($tickets) === 0): ?>
                            <td colspan="<?php echo count($hesk_settings['customer_ticket_list']) + 1 + ($show_view_ticket_column ? 1 : 0); ?>"><span role="alert"><?php echo $hesklang['no_results_found']; ?></span></td>
                        <?php endif; ?>
                        <?php foreach ($tickets as $ticket):
                            ?>
                            <tr <?php if (intval($ticket['status_id']) === 2) { echo 'class="new"'; } ?>>
                                <?php
                                    // Show Sequential ID; link it only if tracking ID and subject are unavailable
                                    if ( in_array('id', $hesk_settings['customer_ticket_list']) )
                                        if ($hesk_settings['sequential']): ?>
                                    <td>
                                        <?php if ($link_sequential_id): ?>
                                        <a href="ticket.php?track=<?php echo rawurlencode($ticket['trackid']); ?>">
