...
function print_add_ticket()
{
    ...
    if (isset($_REQUEST['priority'])) {
        $requested_priority = hesk_REQUEST('priority');
        if (hesk_is_valid_priority_id($requested_priority)) {
            $_SESSION['c_priority'] = intval($requested_priority);
        }
    }
...
function forgot_tid()
{
    ...
    // Rate-limit ticket ID reminder requests; successful reminders intentionally do not reset the counter
