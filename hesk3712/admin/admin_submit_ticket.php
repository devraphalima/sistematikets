...
// Ensure that the customer exists, isn't pending approval, and doesn't
// conflict with an account that owns the same email address.
if (hesk_get_selectable_customer_by_id($tmpvar['customer_id']) === null) {
...
foreach ($tmpvar['follower_ids'] as $follower_id) {
    ...
    $follower_record = hesk_get_selectable_customer_by_id($follower_id_int);
    if ($follower_record && $follower_record['email'] && !hesk_isBannedEmail($follower_record['email'])) {
