...
if ($predefined_name !== '' && $predefined_email !== '') {
    $existing_customer_id = hesk_get_or_create_customer($predefined_name, $predefined_email, false);
}

// Do not restore or preselect customers that cannot be assigned to a new ticket.
if ($existing_customer_id !== null && hesk_get_selectable_customer_by_id($existing_customer_id) === null) {
    $existing_customer_id = null;
}

if ($predefined_name !== '' && $existing_customer_id === null) {
...
<div class="main__content categories ticket-create">
    <div class="table-wrap">
        ...
        <form method="post" class="form <?php echo isset($_SESSION['iserror']) && count($_SESSION['iserror']) ? 'invalid' : ''; ?>"
            ...
            // Load only followers that can still be assigned to a new ticket
            if ( ! empty($hesk_settings['multi_eml'])) {
                $restored_follower_ids = array();

                foreach (hesk_SESSION_array('as_follower_ids') as $follower_id) {
                    $follower_id = intval($follower_id);

                    if ($follower_id > 0) {
                        $restored_follower_ids[$follower_id] = $follower_id;
                    }
                }

                $selectable_followers = hesk_get_selectable_customers_by_ids($restored_follower_ids);

                foreach ($restored_follower_ids as $follower_id) {
                    if (isset($selectable_followers[$follower_id])) {
                        $follower = $selectable_followers[$follower_id];
                        $session_followers[] = array(
                            'id' => $follower['id'],
                            'name' => $follower['name'],
