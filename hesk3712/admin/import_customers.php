...
// Create a scoped authorization token; its lifetime starts when the import begins
if (empty($_SESSION['customer_import_token']) ||
    (!empty($_SESSION['customer_import_token_expires']) && $_SESSION['customer_import_token_expires'] < time())) {
    $_SESSION['customer_import_token'] = bin2hex(random_bytes(32));
    unset($_SESSION['customer_import_token_expires']);
}
$customer_import_token = $_SESSION['customer_import_token'];
...
<script>
    ...
    const importDropzone = new Dropzone('#upload_filedrop', {
        ...
        accept: function(file, done) {
            ...
            reader.addEventListener('loadend', function(event) {
                ...
                UPLOAD.contents = UPLOAD.contents.filter(function(row, index) {
                    return index === 0 || row.some(function(value) {
                        return /[\p{L}\p{N}]/u.test(value.replace(/[\u115F-\u11FF\u3164\uD7B0-\uD7FF\uFFA0]/gu, ''));
                    });
                });
    ...
    function doUpload(index) {
        ...
        const requestBody = {
            ...
            import: 1,
            import_token: '<?php echo hesk_makeJsString($customer_import_token); ?>',
            ...
            error: function(err) {
                ...
                try {
                    ...
                    response = {
                        ...
                        message: '<?php echo hesk_makeJsString($hesklang['attachment_upload_error']); ?>'
                    };
                }
                customerStatus.innerHTML = '<?php echo hesk_makeJsString($hesklang['error']) ?> - ';
                customerStatus.innerHTML += response.message;
                customerStatus.classList.add('failed');
                incrementFailed();

                if (err.status === 401 || err.status === 403 || err.status >= 500 || ['CUSTOMER_ACCOUNTS_DISABLED', 'PERMISSION_DENIED', 'IMPORT_AUTHORIZATION_EXPIRED', 'IMPORT_INTERRUPTED'].indexOf(response.code) !== -1) {
                    const importStatus = document.querySelector('#step-3-pending');
                    importStatus.textContent = response.message;
                    importStatus.classList.remove('blue');
                    importStatus.classList.add('orange');
                    if (response.code === 'IMPORT_AUTHORIZATION_EXPIRED') {
                        importStatus.appendChild(document.createElement('br'));
                        const loginLink = document.createElement('a');
                        loginLink.href = 'import_customers.php';
                        loginLink.textContent = '<?php echo hesk_makeJsString($hesklang['click_login']); ?>';
                        importStatus.appendChild(loginLink);
                    }
                    finishImport();
                    return;
                }
