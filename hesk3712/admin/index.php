...
function do_login()
{
	...
	hesk_limitBfAttempts(true, 'STAFF');
    ...
    hesk_session_stop();
    hesk_session_start();
    $_SESSION['HESK_USER'] = $user_row['user'];
    $_SESSION['id'] = $user_id;
    $_SESSION['mfa_enrollment'] = $mfa_enrollment;
    $_SESSION['mfa_account_enrollment'] = intval($user_row['mfa_enrollment']);
    $_SESSION['mfa_required'] = intval($hesk_settings['require_mfa']);
    $_SESSION['mfa_session_verify'] = hesk_activeSessionCreateTag($user_row['user'], $user_row['pass']);
    $_SESSION['mfa_expires'] = time() + 20 * 60;
...
function do_mfa_verification() {
    ...
    require(HESK_PATH . 'inc/mfa_functions.inc.php');
    $user_row = get_pending_mfa_user_or_fail();

    if (($_SESSION['mfa_verify_option'] === 1 && !is_mfa_email_code_valid($user_row['id'], hesk_POST('verification-code'))) ||
        ($_SESSION['mfa_verify_option'] === 2 && !is_mfa_app_code_valid($user_row['id'], hesk_POST('verification-code')))) {
    ...
    process_successful_login(get_pending_mfa_user_or_fail());
}

function get_pending_mfa_user_or_fail() {
    global $hesk_settings, $hesklang;

    if (empty($_SESSION['id']) || empty($_SESSION['HESK_USER']) || empty($_SESSION['mfa_session_verify']) ||
        empty($_SESSION['mfa_expires']) || !isset($_SESSION['email'], $_SESSION['mfa_enrollment'],
        $_SESSION['mfa_account_enrollment'], $_SESSION['mfa_required'], $_SESSION['mfa_verify_option']) ||
        time() > intval($_SESSION['mfa_expires'])) {
        hesk_session_stop();
        hesk_session_start();
        hesk_process_messages($hesklang['wrong_user'], 'index.php');
    }

    $result = hesk_dbQuery("SELECT * FROM `".hesk_dbEscape($hesk_settings['db_pfix'])."users` WHERE `id` = ".intval($_SESSION['id'])." AND `active` = 1 LIMIT 1");
    ...
    $user_row = hesk_dbFetchAssoc($result);
    $mfa_enrollment = intval($user_row['mfa_enrollment']);
    if (empty($mfa_enrollment) && $hesk_settings['require_mfa']) {
        $mfa_enrollment = 1;
    }
    $mfa_verify_option = intval($_SESSION['mfa_verify_option']);

    if (!in_array($mfa_enrollment, array(1, 2), true) || !in_array($mfa_verify_option, array(1, 2), true) ||
        ($mfa_enrollment === 1 && $mfa_verify_option !== 1) ||
        !hesk_activeSessionValidate($user_row['user'], $user_row['pass'], $_SESSION['mfa_session_verify']) ||
        $user_row['user'] !== $_SESSION['HESK_USER'] || $user_row['email'] !== $_SESSION['email'] ||
        intval($user_row['mfa_enrollment']) !== intval($_SESSION['mfa_account_enrollment']) ||
        intval($hesk_settings['require_mfa']) !== intval($_SESSION['mfa_required']) ||
...
function do_backup_code_verification() {
    ...
    if (!verify_mfa_backup_code($user_row['id'], hesk_POST('backup-code'))) {
        hesk_process_messages($hesklang['mfa_invalid_backup_code'], 'NOREDIRECT');
        // Invalid attempts increase the lockout limit
        hesk_limitBfAttempts(true, 'STAFF');
        $_SESSION['remember_user_form_val'] = hesk_POST('remember_user');
        print_mfa_verification(true);
        exit();
    }
...
function process_successful_login($user_row) {
    ...
    // Reset repeated emails and any reset-only session data
    hesk_cleanSessionVars('mfa_emails_sent');
    hesk_cleanSessionVars('HESK_USER');
    hesk_cleanSessionVars(array('mfa_account_enrollment', 'mfa_required', 'mfa_session_verify', 'mfa_expires'));
    hesk_cleanSessionVars(array('password_reset', 'password_reset_user_id', 'password_reset_hash', 'password_reset_ip', 'password_reset_expires'));

    /* Login successful, clean brute force attempts */
    hesk_cleanBfAttempts('STAFF');
            ...
            <div class="reg__wrap">
                ...
                <div class="reg__section">
                    <div class="reg__box">
                        ...
                        <form action="index.php" class="form <?php echo isset($_SESSION['a_iserror']) && count($_SESSION['a_iserror']) ? 'invalid' : ''; ?>"
                              id="form1" method="post" name="form1" novalidate aria-label="<?php echo $hesklang['login']; ?>">
                            ...
                            <div class="form__submit">
                                ...
                                {
                                    echo '<input type="hidden" name="goto" value="' . hesk_htmlspecialchars($url) . '">';
                                }
                                ?>
                            </div>
...
function print_mfa_verification($show_backup_code = false)
{
    global $hesk_settings, $hesklang;
                    ...
                    <div class="reg__wrap">
                        ...
                        <div class="reg__section">
                            <div class="reg__box">
                                ...
                                <form action="index.php" class="form <?php echo isset($_SESSION['a_iserror']) && count($_SESSION['a_iserror']) ? 'invalid' : ''; ?>"
                                      id="form1" method="post" name="form1" novalidate aria-label="<?php echo $hesklang['mfa']; ?>">
                                ...
                                <?php if ($_SESSION['mfa_verify_option'] === 1): ?>
                                    &nbsp;
                                    <form action="index.php" class="form <?php echo isset($_SESSION['a_iserror']) && count($_SESSION['a_iserror']) ? 'invalid' : ''; ?>"
                                          id="send-another-email-form" method="post" name="send-another-email-form" novalidate aria-label="<?php echo $hesklang['mfa_send_another_email']; ?>">
                                        ...
                                        <li>
                                            <div class="flex">
                                                ...
                                                <div class="mfa-alt-text">
                                                    <form action="index.php" class="form <?php echo isset($_SESSION['a_iserror']) && count($_SESSION['a_iserror']) ? 'invalid' : ''; ?>"
                                                          id="email-backup-form" method="post" name="email-backup-form" novalidate aria-label="<?php echo $hesklang['mfa_verify_another_way_email']; ?>">
                                                        <button class="btn btn-link" type="submit">
                                            ...
                                            <div class="flex">
                                                ...
                                                <div class="mfa-alt-text">
                                                    ...
                                                    <div id="backup-code-field" style="display: <?php echo $show_backup_code ? 'block' : 'none'; ?>">
                                                        <form action="index.php" class="form <?php echo isset($_SESSION['a_iserror']) && count($_SESSION['a_iserror']) ? 'invalid' : ''; ?>"
                                                              id="backup-form" method="post" name="backup-form" novalidate aria-label="<?php echo $hesklang['mfa_backup_code']; ?>">
                                                            <div class="form-group">
