...
if (isset($_GET['delnote']) && hesk_token_check())
{
    ...
    {
		// Get note info
		$res = hesk_dbQuery("SELECT `who`, `attachments` FROM `".hesk_dbEscape($hesk_settings['db_pfix'])."notes` WHERE `id`={$n} AND `ticket`='".intval($ticket['id'])."'");
		...
		if ( hesk_dbNumRows($res) )
		{
			...
			{
				// Delete note
				hesk_dbQuery("DELETE FROM `".hesk_dbEscape($hesk_settings['db_pfix'])."notes` WHERE `id`='".intval($n)."' AND `ticket`='".intval($ticket['id'])."'");
...
if (isset($_GET['bm_add']) && hesk_token_check()) {
    ...
    } elseif ($_GET['bm_add'] == 0 && ! empty($ticket['is_bookmark'])) {
...
function hesk_staffCanAccessTicketNoError($ticket)
{
    ...
    if ( ! is_array($ticket) || ! isset($ticket['id'], $ticket['category'], $ticket['owner']) || ! array_key_exists('assignedby', $ticket))
    {
        return false;
    }
