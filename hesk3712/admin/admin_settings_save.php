...
if ($section === 'GENERAL') {
	...
	elseif ($mysql_log)
	{
		hesk_error(nl2br(hesk_htmlspecialchars($mysql_error)) . '<br /><br /><b>' . $hesklang['mysql_said'] . ':</b> ' . hesk_htmlspecialchars(hesk_mb_substr($mysql_log, 0, 32768)));
	}
	else
	{
		hesk_error(nl2br(hesk_htmlspecialchars($mysql_error)));
	}
...
if ( ! $smtp_OK)
{
    $tmp[] = '<span class="text-danger">'.$hesklang['sme'].':</span> '.hesk_htmlspecialchars($smtp_error).'<br /><br /><a href="Javascript:void(0)" onclick="Javascript:hesk_toggleLayerDisplay(\'smtplog\')">'.$hesklang['scl'].'</a><div id="smtplog" style="display:none">&nbsp;<br /><textarea name="log" rows="10" cols="60">'.hesk_htmlspecialchars(hesk_mb_substr($smtp_log, 0, 32768)).'</textarea></div>';
}

if ( ! $pop3_OK)
{
    $tmp[] = '<span class="text-danger">'.$hesklang['pop3e'].':</span> '.hesk_htmlspecialchars($pop3_error).'<br /><br /><a href="Javascript:void(0)" onclick="Javascript:hesk_toggleLayerDisplay(\'pop3log\')">'.$hesklang['pop3log'].'</a><div id="pop3log" style="display:none">&nbsp;<br /><textarea name="log" rows="10" cols="60">'.hesk_htmlspecialchars(hesk_mb_substr($pop3_log, 0, 32768)).'</textarea></div>';
}

if ( ! $imap_OK)
{
    $tmp[] = '<span class="text-danger">IMAP error:</span> '.hesk_htmlspecialchars($imap_error).'<br /><br /><a href="Javascript:void(0)" onclick="Javascript:hesk_toggleLayerDisplay(\'imaplog\')">IMAP connection log</a><div id="imaplog" style="display:none">&nbsp;<br /><textarea name="log" rows="10" cols="60">'.hesk_htmlspecialchars(hesk_mb_substr($imap_log, 0, 32768)).'</textarea></div>';
}
